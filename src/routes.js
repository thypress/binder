...
export async function handleRequest(request, server, deps) {
  ...
  if (honeypotRoutes.includes(route)) {
    const ip = deps.securityManager.getClientIP(request);
 ...
 */
async function handleAdmin(request, deps, adminBase) {
  const url = new URL(request.url);
  const route = url.pathname;
  ...
  if (route === `${adminBase}/setup-pin` && request.method === 'POST') {
    ...
    try {
      ...
      if (!/^\d{4}$/.test(pin)) {
        return new Response(JSON.stringify({ success: false, error: 'PIN must be 4 digits' }), {
          status: 400,
          headers: { 'Content-Type': MIME_TYPES.JSON }
        });
      }
  ...
  if (!securityManager.verifySession(request)) {
